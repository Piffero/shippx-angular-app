...
export class Forgot {
  ...
  async resendCode() {
    ...
    const { error } = await this._auth.resetPasswordForEmail(this.email());
  ...
  async sendResetCode() {
    ...
    try {
      const { data, error } = await this._auth.resetPasswordForEmail(emailValue);
      if (!error) {
  ...
  async verifyCode() {
    ...
    try {
      const { data, error } = await this._auth.verifyOtp(email, code, 'recovery');
      if (error) {
  ...
  async resetPassword() {
    ...
    try {
      const { data, error } = await this._auth.updateUserPassword(newPasswordValue);
      if (error) {
