...
export class Packages implements OnInit, OnDestroy {
  ...
  // Função para carregar créditos via Pix (exemplo de integração com outro serviço)
  async rechargeCredits() {
    try {
      this.isLoading.set(true);
      const charge = await this.paymentService.createPixChange('DELIVERY_CREDIT');
      if (charge?.error) { throw charge.error; }
      this.messageEvent.set('Cobrança Pix de recarga gerado com sucesso!');
    } catch (err: any) {
      this.messageEvent.set("Erro ao gerar PIX de recarga: " + (err.message || 'Tente novamente mais tarde.'));
    } finally {
  ...
  // Nevega para o formulário de criação de pacote, pré-preenchendo o ID do pedido
  createPackageFromOrder() {
    this._router.navigate(['/shippx/shipments/create']);
  }
}
